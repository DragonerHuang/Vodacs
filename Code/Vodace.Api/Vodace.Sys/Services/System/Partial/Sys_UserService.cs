...
namespace Vodace.Sys.Services
{
    public partial class Sys_UserService
    {
        ...
        public Sys_UserService(IHttpContextAccessor httpContextAccessor, ISys_UserRepository repository, ISys_CompanyRepository sys_Company, ISys_MenuRepository menuRepository, ILocalizationService localizationService)
            : base(repository)
        {
        ...
        public async Task<WebResponseContent> Login(LoginInfo loginInfo, bool verificationCode = true)
        {
            ...
            try
            {
                ...
                {
                    companyId = _companyRepository.Find(d => d.company_no == loginInfo.Com_No).FirstOrDefault()?.id;
                }
                Sys_User user = await repository.FindAsIQueryable(x => (companyId == Guid.Empty || loginInfo.UserName.Equals("admin") ? true : x.Com_Id == companyId) && x.UserName == loginInfo.UserName)
                    .FirstOrDefaultAsync();
                if (user == null)
                {
                ...
                var res_data = new UserInfo()
                {
                    ...
                    Contact_Id = user.Cont_Id,
                    Gender = user.Gender,
                    Lang = user.Lang,
                };
                string token = JwtHelper.IssueJwt(res_data);
                user.Token = token;
                webResponse.data = new { token, userName = user.UserTrueName, img = "", res_data };
                repository.Update(user, x => x.Token, true);

                List<Sys_User> _Users = new List<Sys_User>();
                repository.UpdateRange(_Users, x => new { x.Enable }, true);
                UserContext.Current.LogOut(user.User_Id);
                loginInfo.Password = string.Empty;
                Log4NetHelper.Info($"{user.UserName}{_localizationService.GetString("login_success")}！");
