...
namespace Vodace.Core.ManageUser
{
    public class UserContext
    {
        ...
        public UserInfo GetUserInfo(int userId)
        {
            ...
            _userInfo = DBServerProvider.DbContext.Set<Sys_User_New>()
        ...
        public static int MenuType
        {
            get
            {
                ...
                return Context.Request.Headers.ContainsKey("uapp") ? 1 : 0;
#else
                return Context.Request.Headers.ContainsKey("uapp") ? 1 : 3;
#endif
        ...
        public List<Permissions> GetPermissions(int roleId)
        {
            if (IsRoleIdSuperAdmin(roleId))
            {
                ...
                var permissions = DBServerProvider.DbContext.Set<Sys_Menu>()
                    .Where(x => (x.enable == 1 || x.enable == 2) && x.delete_status ==0)
                    .Select(a => new Permissions
                    {
                        Menu_Id = a.menu_id,
                        ParentId = a.parent_id,
                        //2020.05.06增加默认将表名转换成小写，权限验证时不再转换
                        TableName = (a.table_name ?? "").ToLower(),
                        //MenuAuth = a.Auth,
                        UserAuth = a.auth,
                        // 2022.03.26增移动端加菜单类型
                        MenuType = a.menu_type ?? 0
                    }).ToList();
            ...
            lock (objId)
            {
                ...
                List<Permissions> _permissions = (from a in dbContext.Set<Sys_Menu>()
                                                  join b in dbContext.Set<Sys_Role_Auth>()
        ...
        //{
        //    get { return UserInfo.user_true_name; }
        //}
        public int? Lang 
        {
            get { return UserInfo.Lang; }
        }
        public string Token
        {
