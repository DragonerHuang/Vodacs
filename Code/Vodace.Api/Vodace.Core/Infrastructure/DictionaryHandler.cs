...
namespace Vodace.Core.Infrastructure
{
    public static class DictionaryHandler
    {
        ...
        public static string GetRolesSql(string originalSql)
        {
            ...
            {
                originalSql = $"SELECT \"role_id\" AS \"id\",\"role_id\" AS  \"key\",\"role_name\" AS \"value\" FROM \"Sys_Role\""
                   + $" where \"role_id\"  in ({string.Join(',', roleIds)})";
            }
            else {
                originalSql= $@"SELECT role_id as 'key',role_id AS id,role_name as 'value' FROM Sys_Role 
