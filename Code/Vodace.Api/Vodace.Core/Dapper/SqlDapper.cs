...
namespace Vodace.Core.Dapper
{
    public class SqlDapper : ISqlDapper
    {
        ...
        public IQueryable<T> QueryQueryable<T>(string cmd, object param, CommandType? commandType = null, bool beginTransaction = false)
        {
            return  (IQueryable<T>)Execute((conn, dbTransaction) =>
            {
                return  conn.Query<T>(cmd, param, dbTransaction, commandType: commandType ?? CommandType.Text, commandTimeout: commandTimeout).AsQueryable();
            }, beginTransaction);
        }
