    ...
    public class EPPlusHelper
    {
        ...
        private static List<CellOptions> GetExportColumnInfo(string tableName, bool temlate = false, bool filterKeyValue = true, string[] columns = null)
        {
            ...
            foreach (string dicNo in dicNos.Distinct())
            {
                ...
                List<Sys_Dictionary_List> dictionaryLists = dictionaries
                   .Where(x => x.dic_no == dicNo && x.Sys_DictionaryList != null)
                ...
                foreach (var item in dictionaryLists)
                {
                    ...
                    if (filterKeyValue && item.dic_name == item.dic_value) continue;
                    if (keyValues.ContainsKey(item.dic_value)) continue;
                    keyValues.Add(item.dic_value, item.dic_name);
                }
